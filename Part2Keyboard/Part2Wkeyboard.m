...
'Remember the orientation of the triangle.\n' ...
'Use T to rotate clockwise.\n' ...
'Use B to rotate anticlockwise.\n' ...
'Press 1, 2, 3, or 4 to submit your response.\n\n' ...
...
ALL_SUB = [CWD 'SubjectDataWKeyboard/']; % create "SubjectData" dir that will contain all the data for all subjects
...
% Coding question 2:
% Removed dateStr from taskMapFile naming so the same taskmap
% filename is used for a given subject/session/run. This allows
% MATLAB to detect an existing taskmap and gives the experimenter
...
for   tc = 1:numberOfTrials
    ...
    stimTheta = taskMap(:,tc).triangOrientation;
...
% Coding Challenge Question 7:
% Keyboard dial imitation:
% t = clockwise
% b = counterclockwise
% 1-4 = submit response
...
while ~responseLocked
    ...
    % Question 6: Quit experiment with Q
    if keyIsPressed && keyCode(KbName('q'))
    ...
    if keyCode(KbName('1!')) || ...
       ...
       keyCode(KbName('4$'))
        ...
        responseLocked = 1;
        timeEnd = GetSecs;    % <-- keep this here
    end
...
respError = thetaDeg - taskMap(:,tc).triangOrientation;
respError = mod(respError + 180, 360) - 180;
    ...
    disp(['Actual: ' num2str(taskMap(:,tc).triangOrientation)])
